...
app.get('/states/', async (request, response) => {
  ...
  response.send(
    statesArray.map(stateobject => convertStateNamePascalCase(stateobject)),
  )
})
...
app.get('/districts/:districtId/details/', async (request, response) => {
  const {districtId} = request.params
  const getDistrictQuery = `SELECT state_name AS stateName FROM district INNER JOIN state ON district.state_id=state.state_id WHERE district_id=${districtId};`
